...
<p> 패키지 벡터와 낱개 벡터를 나누어서 입력을 받고 각각 정렬을 시킨다. 그러면 최소의 패키지 가격과 최소의 낱개 가격이 나온다. 이 때 최소의 낱개가 여러 개 사는 것이 더 쌀 수도 있기 때문에 이를 고려해서 패키지 1세트씩 늘리면서 최소의 가격을 구한다.</p>
