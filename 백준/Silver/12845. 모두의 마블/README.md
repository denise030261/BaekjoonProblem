...
<p>문제를 보면 카드를 붙일 때는 골드를 합치지만 카드의 레벨은 유지하기 때문에 최대한 높은 레벨은 남겨야한다. 즉, 입력을 받으면서 어떤 카드가 제일 높은지와 순서를 알아내서 이 카드를 기준으로 인접한 카드의 레벨을 합해서 골드를 얻으면 되는 그리디의 문제이다. 인접한 카드는 하나의 레벨로 유지하니 처음부터 쭉 높은 레벨을 더해주면 된다.</p>
