...
### 문제 풀이
 ...
 <p> 이 문제는 그리디 문제로 문제를 읽어보면 가장 왼쪽에서 정수만큼 떨어진 거리만 물이 샌다고 되어있다. 그리고 이것을 테이프로 이용해서 물을 막으려고 한다. 이를 위해서 본인은 우선순위 큐를 사용하였다. 내림차순으로 세워서 기준으로부터 L까지 큐에 있으면 빼고 없으면 다음 순서로 넘어가는 식으로 구하였다. 기준(첫번째 while문)을 세울 때마다 테이프의 수를 더해 답을 구한다. </p>
