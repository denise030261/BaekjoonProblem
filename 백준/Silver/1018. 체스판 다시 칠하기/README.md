...
### 문제 풀이
 ...
 <p> 이 문제는 8x8로 잘라 '흰검 패턴'이나 '검흰 패턴'으로 칠할 때 최소한의 색칠 개수를 구하는 것이다. 먼저 흰검일 때의 배열과 검흰일 때의 배열을 먼저 선언하여 8x8씩 구역을 나누어서 이 배열들을 비교하고 다르면 count를 더해간다. 그 중에서 count의 가장 작은 값이 이 문제의 답이 되는 것이다. </p>
