...
### 문제 풀이
 ...
 <p>이 문제는 두 카드를 골라서 합하고 그 두 카드는 합한 값이 된다. 그래서 m번을 한 뒤 가장 작은 점수를 구해야한다. 이 때 가장 작은 점수를 구하기 위해서는 가장 작은 값들을 더해야하고 카드를 골라 합한 값도 바로 반영해야 하기 때문에 우선순위 큐를 사용해서 값들을 더하고 답을 도출하면 된다.</p>
