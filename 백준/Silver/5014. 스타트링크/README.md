...
### 문제 풀이
 ...
 <p>이 문제는 강호가 있는 층 S층에서 스타트링크가 있는 G층으로 가는 문제로 bfs로 풀었다. 엘레베이터로 위아래로 갈 수 있기 때문에 위아래로 갈 수 있는지 확인하면서 너비 탐색을 하고 그 중에 최솟값을 구하여 G층으로 가는 최솟값을 출력하면 된다. </p>
