...
### 문제 풀이
 ...
 <p>본인이 문제를 풀 때는 배열부터 큰 숫자의 위치를 기억했다가 계산을 할 때는 그 위치에서부터 0까지 역순으로 계산을 하였는데 이 문제의 의도는 애초에 역순부터 서치하여 큰 숫자가 있으면 이를 기준으로 다른 배열 요소들을 계산해주고 이보다 큰 숫자가 있으면 그 숫자로 바꾸어서 계산을 한다.</p>
