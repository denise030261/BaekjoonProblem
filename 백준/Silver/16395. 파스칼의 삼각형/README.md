...
### 문제 풀이
 ...
 <p>파스칼의 삼각형 문제로 만드는 방법을 보면서 문제를 풀이하면 된다. 이중for문으로 첫 번째 행과 두 번째 행부터 양 끝같은 1로 처리한다. 나머지는 바로 위 행의 인접한 두 수의 합이라고 했으니 tri[i][j]=tri[i-1][j]+tri[i-1][j-1]의 식으로 구할 수 있다. 그래서 입력된 수의 위치에 있는 답을 출력하면 된다.</p>
