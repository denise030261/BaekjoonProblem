...
### 문제 풀이
 ...
 <p>이 문제는 유의해야할 것이 있다. 박스가 최대 넣을 수 있는 무게가 있고 답은 숌이 필요한 박스의 개수의 최솟값을 구하는 것인데 이 때 차례대로 박스에 책을 넣어야한다. 즉, 입력 순서대로 넣어야 하고 포장이 완료되면 다시 그 박스에 넣을 수 없다. 이를 유의하고 박스에 책을 넣으면서 최대 무게가 넘기느냐 아니냐만 확인하면 된다. 최대 무게가 넘기면 새로운 박스를 아니면 그 박스에 넣는 단순한 문제가 되는 것이다.</p>
