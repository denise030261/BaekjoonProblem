...
### 문제 풀이
 <p>이 문제는 구현 문제로 총 3단계를 거친다. 첫 번째는 회전을 해주는데 k의 숫자가 클 수 있기 때문에 나머지 연산을 처리해서 회전을 해준다. 그 다음에 인접하는 수가 있는 확인한다. 이 때 인접하는 수는 4방향으로 찾되 가로에서 맨 처음과 맨 끝은 연결된다는 점을 생각하고 인접하는 수를 찾아야 한다. 그 다음에 없애주고 만약에 없애주는 수가 없으면 평균을 처리하여 답을 구하되 평균보다 큰 수에서 1을 빼고 작은 수에는 1을 더한다고 하는데 이 때 같은 수면 아무 언급이 없으므로 이를 처리하면 안된다. 그렇게 해서 T번을 해주면 답을 도출할 수 있다. </p>
