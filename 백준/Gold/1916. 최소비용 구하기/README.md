...
### 문제 풀이
 ...
 <p> 주어진 입력으로 A번째 도시에서 B번째 도시 가는데 드는 최소 비용을 구하는 문제로 다익스트라 방식으로 문제를 풀면된다. 출발 지점을 두고 우선순위 큐를 사용하여 최솟값이 top에 있어야하기 때문에 push를 할 때 비용에 마이너스를 붙여서 B번째 도시까지 가는데 최소 비용을 구한다.</p>
