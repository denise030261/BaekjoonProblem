...
### 문제 풀의
 <p>이 문제는 투 포인터로 먼저 정렬을 한 뒤에 start와 end를 맨 처음으로 둔다. 그 뒤에 둘의 위치에 있는 값과 차이를 비교하면서 M이상이지만 가장 작은 차이를 출력하도록 한다. 만약 차이가 M보다 크면 start를 높이고 작으면 end를 높인다.</p>
