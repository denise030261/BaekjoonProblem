...
<p>이 문제는 bfs로 풀어야한다. 처음에 dfs로 풀었지만 메모리초과가 났고 bfs로 방문처리 배열을 파란 구슬 따로 빨간 구슬 따로 구했는데 그렇게 하면,  빨간 구슬이 같은 위치이고 파란 구슬이 다른 위치일 때 다른 위치인건데 빨간색은 같은 위치로 처리되기 때문에 4차원 배열을 만들어서 구해야한다. 또한, 두 구슬 굴러야하기 때문에 위치 배치를 먼저 둘 다 독립적으로 구하되 기울여서 위치를 구하면 같을 때 구른 거리가 짧다는것은 먼저 도착했다는 의미로 길게 구른 구슬을 이동하는 방향에서 하나 빼준다. </p>
