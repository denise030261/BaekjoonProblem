...
### 문제 풀이
 <p>이 문제는 이해부터 힘들었다.자세히 보니 1부터 gi번째 인 것을 알았고 그 다음도 문제였다. 어떻게 배치하냐가 문제였다. 그래서 그리디로 도킹하려는 숫자 중에 큰 것부터 배치하도록 했지만 그러면 시간초과가 났다. 그래서 추가해야하는 것이 Union-Find 알고리즘이다. root의 역할을 비어있는 게이트를 가리키도록 하였다. 그래서 union할 때 find에서 게이트가 배치되고 그 다음에 비어있는 게이트를 가리키도록 구현하였다. Union(plane,plane-1)</p>
