...
### 출력 
 ...
 ### 문제 풀이
  <p>이 문제는 브루트와 bfs 조합으로 풀어야한다. 모든 바이러스의 위치를 파악하고 그 중 어떤 바이러스를 활성화 시킬지 재귀로 구해준다. 그 다음에 바이러스를 퍼뜨릴 최소 시간을 구해야하는데 이 때 우선순위 큐를 사용하여 시간이 제일 낮은 순부터 바이러스를 퍼뜨려서 만약 안전 영역이 다 바이러스에 감염되었다고 하면 그 시간 중에서 최소 시간을 구해준다. 이 때 비활성화된 바이러스가 활성화가 되면 바이러스가 활성화되므로 이를 주의해야 한다. </p>
