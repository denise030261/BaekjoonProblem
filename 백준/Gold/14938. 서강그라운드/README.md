...
### 문제 풀이
 ...
 <p>이 문제는 다익스트라 알고리즘으로 문제 풀이하였다. 처음 문제를 풀었을 때 어느 지역에 수색을 완료하면 visited를 처리하여 다음에는 방문하지 않도록 하였다. 하지만 그렇게 하면 짧은 길로 수색했던 곳을 지나서 다른 곳으로 이동하려고 할 때 방문처리된 것 때문에 지나가지 못하게 된다. 그래서 방문처리를 하면 안되고 아이템을 먹는것으로 처리하여 수색완료하면 그 지역의 아이템은 0으로 처리해야 한다.</p>
