...
### 문제 풀이
 <p>처음 문제를 풀 때 섬마다 태그를 달아줘야하는 것을 깨달아 dfs로 섬을 나눠줬다. 그리고 짧은 거리를 구할 때 dfs로 구했으나 시간 초과가 났고 두 섬의 거리를 하나씩 구해보자는 생각을 하여 구했더니 풀게 되었다. 하지만 범위가 넓어지면 시간초과가 나기 때문에 bfs로 풀어야한다. 이 때 바다쪽에 있는 육지를 큐에 넣고 bfs 방식으로 풀어야 한다.</p>
