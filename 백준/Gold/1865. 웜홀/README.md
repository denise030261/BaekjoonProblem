...
### 문제 풀이
 <p>이 문제는 벨만-포드 알고리즘을 사용하여 문제를 풀면 된다. 왜냐하면 웜홀 내에서는 시간이 거꾸로 간다고 한다. 즉 음수의 값을 가진 간선이 존재한다는 이야기이다. 그리고 다시 시작 위치로 갔을 때 시간이 되돌아가는 경우를 알고 싶어했는데 즉 음의 순환을 가지면 시간이 되돌아가기 때문에 벨만-포드 알고리즘을 사용하면 된다.</p>
