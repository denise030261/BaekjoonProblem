...
### 문제 풀이
 <p>이 문제는 구현 문제로 3단계로 먼저 궁수를 배치해준다. 궁수 배치할 때 dfs를 사용하여 궁수를 배치하였다. 궁수를 배치한 뒤 각 궁수와 가까운 거리의 적이 있는지 파악하고 쏜다. 이 때 D보다 적어야 하고 가장 가까우며 왼쪽부터 쏴야하기 때문에 이중문에서 첫 번째 반복문에서 2차배열을 돌리게 한다. 그렇게해서 왼쪽부터 찾아보고 적이 있으면 쏜다. 다 쏘면 남아있는 적을 아래로 내려오게 한다. </p>
