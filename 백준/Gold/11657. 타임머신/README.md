...
### 문제 풀이
 <p>이 문제는 가중치가 음수이기 때문에 벨만-포드 알고리즘을 사용해야한다. 그래서 한 번 간선을 돌아 업데이트를 하고 또 한 번 도는데 음수가 있다면 음수의 순환이 있다는 이야기로 -1을 출력해서 끝내고 그것이 아니라면 각 노드의 비용을 출력하면 된다. </p>
