...
### 문제 풀이
 ...
 <p>이 문제는 플로이드-워셜 알고리즘을 사용하여 문제를 푼다. 이 문제는 회원을 노드로 생각하면 된다. 점수를 가장 적은 사람이 회장이 되는건데 즉, 각 노드 중에 최대값 중에 제일 최솟값을 가진 노드를 구하는 것이다. 그렇게 때문에 모든 노드의 경로를 구해야해서 플로이드-워셜 알고리즘을 사용해야하는 것이다. </p>
