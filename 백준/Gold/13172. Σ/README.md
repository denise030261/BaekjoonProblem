...
<p>이 문제는 페르마의 소정리를 이용해서 모듈러 역수를 구하는 것이 목표이다. 문제에 나와있는대로 1000000007은 소수이기 때문에(소수이어야 기약분수를 구할 수 있다) 페르마의 소정리인 b<sup>X - 2</sup> ≡ b<sup>-1</sup> (mod X) 을 이용하여 모듈러 역수를 구하는데 이 때 이진 제곱법을 사용하면서 구해준다. 그렇게 모듈러 역수를 구하게 되면 S*N^-1을 계산하여 답을 구해준다.</p>
