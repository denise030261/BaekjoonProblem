...
### 문제 풀이 
 <p>이 문제는 정수들이 있을 때 주어진 구간에 최솟값을 구하는 문제로 세그먼트 트리를 이용해 문제를 풀면 된다. build 함수를 통해서 각 구간의 최솟값을 먼저 구한 다음에 구간이 주어지면 query함수를 통해서 그 구간의 최솟값을 출력한다. </p>
