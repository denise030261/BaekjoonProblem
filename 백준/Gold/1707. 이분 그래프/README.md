...
### 문제 풀이
 ...
 <p>이 문제는 이분 그래프로 bfs를 사용하여 풀었다. bfs를 사용할 때 출발하는 점을 1의 그룹으로 표시하고 노드1과 연결되는 노드는 2, 다시 그 노드의 노드는 1... 방식으로 진행하였다. 이 때 연결 노드가 색깔이 같으면 이분 그래프가 아니므로 루프에서 벗어난다. 또한 모든 노드가 연결되어 있다는 보장은 없으므로 모든 노드를 살펴봐야한다.</p>
