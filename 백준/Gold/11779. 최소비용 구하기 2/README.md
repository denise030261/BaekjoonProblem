...
### 문제 
 <p>이 문제는 다익스트라 알고리즘으로 문제를 풀었다. 출발 노드와 도착 노드는 정해져있기 때문에 출발노드를 중점으로 길을 찾으면 된다. 이 때 도착 노드의 최소 비용을 갖는 경로도 찾아야하기 때문에 경로를 찾을 때 비용, 다음 노드, 현재 노드에서 다음 노드까지의 경로를 저장하면서 priority_queue를 사용하면서 문제를 풀었다.</p>
