...
### 문제 풀이
 <p>이 문제는 다익스트라 알고리즘으로 풀었다. 다른 점이 있다면 일반적인 다익스트라 알고리즘은 최소의 비용을 드는 문제이지만 이 문제는 한 번의 이동에서 옮길 수 있는 물품들의 중량의 최댓값을 구하는 문제이다. 즉, 최대값을 구하되 공장에서 공장으로 이동할 때 다리가 견딜 수 있는 무게이어야 한다. 그래서 우선순위 큐에서 정렬을 비용이 내림차순으로 하되 큐에 할당 될 때 현재 비용과 다음으로 가는 비용 중에 최솟값을 할당을 한다. 그래야 두 다리가 견딜 수 있다는 의미이기 때문이다.</p>
