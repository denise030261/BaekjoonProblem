...
### 문제 풀이
 <p>이 문제는 그리디로 풀어야한다. 먼저 정렬을 한 뒤에 순서를 배치한다. 순차적으로 배치되어있는 종료날짜와 현재의 시작날짜를 비교를 해서 이을지 아니면 다음 폭에 배치할지 결정을 한다. 이 때 지금까지의 처음점과 끝점을 비교한다. 끝점과 현재의 시작날짜가 2일 이상 차이가 나면 그 때는 모든 일정이 연속되지 않기 때문에 현재 폭, 처음점, 끝점을 이용해서 직사각형을 구한다. 그렇게 구한 다음에는 리셋해서 다시 처음부터 구해준다.  </p>
