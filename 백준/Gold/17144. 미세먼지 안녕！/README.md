...
### 출력 
 ...
 ### 문제 풀이
  ...
  <p> 이 문제는 구현 문제로 별다른 알고리즘을 사용할 필요는 없다. 그래서 먼저 확산을 할 때의 맵을 구한다. 확산 할 때 가장자리가 아닌지 공기청정기 유무에 따라서 확산의 방향 개수와 양이 다르기 때문에 이를 판별하면서 확산의 방향 개수를 구하고 확산의 양은 식으로 구한다. 구한 식은 확산맵에 옮기고 다 구하면 기존의 맵으로 옮긴다. 그 다음은 주어진 시간동안 공기청정기의 바람이 불 때 미먼지를 구한다. 공기청정기가 위에 있을 때와 아래에 있을 때 부는 바람이 다르기 때문에 이를 유의해서 따로 미세먼지를 구하고 총 미세먼지의 합을 구한다.  </p>

![KakaoTalk_20240716_232706874](https://github.com/user-attachments/assets/960de885-0bdd-46e8-84ae-f9aaf39a9e94)
