...
### 문제 풀이
 <p>이 문제는 그리디를 사용하여 문제를 풀었다. 보석과 가방을 올림차순으로 정렬을 해준다. 가방의 for문을 돌리면서 보석이 가방보다 가벼우면 우선순위 큐에 넣어준다. 만약 보석이 무거우면 그 순서를 index에 저장하고 빠져나온다. 우선순위 큐에서 가장 가치가 높은 값을 훔치는 것이다. 그 다음 가방에서는 이전에 저장한 index부터 다시 보석을 살펴본다. 이 때 보석을 끝까지 살펴봤다면 이에 대한 처리도 해줘야한다. </p>
