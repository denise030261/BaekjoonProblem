...
### 문제 풀이
 <p>이 문제는 누적합과 정렬을 사용하여 문제를 풀어야 한다. 처음 풀 때 문제 이해를 잘못해서 모든 공들의 개수로 구하다가 크기의 합인 것을 보고 누적합을 사용해서 풀었다. 먼저 크기, 색깔, 인덱스를 할당하되 입력을 받으면서 총 크기의 합과 각 색깔별로의 크기의 합을 구한다. 그 다음에 배열에서 정렬을 한 뒤 누적합을 사용하여 총 크기의 합에 현재 공의 크기를 빼주고 또한, 현재 공 색깔의 크기를 현재 공의 크기를 빼준다. 그 뒤에 총 크기의 합에서 현재 공 색깔의 크기를 빼주는데 문제는 다음 공의 크기도 같을 때이다. 같을 때도 앞에서 했듯이 총 크기랑 그 색깔의 크기를 빼주는데 그 다음 크기랑 다를 때까지 빼준다. 현재 순번은 변하지 않는다. 그리고 이미 빼주었다고 표시를 하면서 답을 구해준다. </p>
