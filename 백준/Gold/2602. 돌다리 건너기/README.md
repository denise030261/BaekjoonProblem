...
### 문제 풀이
 <p>이 문제는 dp를 이용하여 문제를 풀었다. 문자열을 맞추기 위해서는 다리를 건너가면서 통과해야하기 때문에 두 개의 2차원 dp배열을 만들었다. 1차원은 현재 다리의 인덱스, 2차원은 두루마리의 인덱스를 저장하였다. 그래서 현재 다리의 문자가 두루마리의 인덱스가 맞으면 같은 다리의 이전문자의 현재 두루마리의 인덱스와 건너편의 이전문자의 이전 두루마리의 인덱스의 배열 값을 합하여 답을 구하였다. 다르다면 같은 다리의 이전 문자의 현재 두루마리의 인덱스의 배열값을 가져온다. 그렇게 하여 악마와 천사의 돌다리 마지막 인덱스이면서 두루마리의 끝인 인덱스를 합하여 구하였다. </p>
