...
### 문제 풀이
 <p>이 문제는 배열을 회전을 하면서 그래프 탐색도 해야하는 문제이다. 파이어스톰을 시전할 때 격자를 2<sup>L</sup> × 2<sup>L</sup> 크기의 부분 격자로 나누고 회전한다고 했기 때문에 해당 격자 숫자를 받아준다. 그리고 y좌표의 끝부분에서 x좌표를 먼저 반복문을 돌아주면서 그 숫자들을 채워준다. 그렇게 하면 90도로 회전이 된다. 그리고 얼음의 양을 줄여야하는지 확인하기 위해서 상하좌우 얼음이 있는지 파악하고 3구간 미만에 얼음이 있으면 해당 구간은 녹게 만든다. 그리고 BFS를 통해서 얼음 덩어리를 구해준다. 이 덩어리를 구할 때 말 그대로 덩어리를 구하는 것이기 때문에 이를 주의해서 푼다.</p>
