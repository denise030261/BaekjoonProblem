...
### 문제 풀이
 <p>이 문제는 특별한 알고리즘 없이 구현으로 문제 풀면 된다. 그래서 순서대로 놓을 큐와 해당 위치에 어떤 공들이 있는지 파악할 큐를 만들어두었다. 그리고 pair<int,int> 대신에 index 값으로 i*N+J 형태로 위치를 표현하였다. 그렇게해서 문제에 나와있는 것처럼 이동할 때마다 (현재위치+(움직일양%견적)+견적)%견적으로 움직여준다. 그래야 음수값 또는 N을 넘어서는 값이 나오지 않는다. 다 움직였으면 모든 공간에 있는 공들이 2개 이상인지 파악한다. 2개 이상이면 문제에 나와있는 것처럼 질량과 속력을 따로 구해준다. 모두 홀수인지 짝수 인지 파악하기 위해서는 맨 처음만 홀수인지 짝수인지 파악하고 그 이후의 숫자와 비교하여 방향들을 결정하였다. 그렇게 해서 구한 불들은 다시 순서 큐에 넣어준다. 마지막까지 다 하면 남은 불의 무게를 합하여 답을 출력한다.  </p>
