...
### 문제 풀이
 <p>이 문제는 중위 표기식을 후위 표기식으로 바꾸는 문제로 스택을 이용하여 문제를 풀면된다. 입력을 스캔할 때 피연산자는 무조건 출력을 한다. 연산자는 우선순위에 따라서 달라지는데 먼저 '(' 무조건 스택에 넣는다. *, /, +, -은 스택의 top과 비교하여 우선순위가 같거나 높으면 출력을 하고 pop을 한다. 이를 반복하고 우선순위가 낮으면 push를 하고 마무리한다. 만약 현재 스캔이 ')'라면 스택의 top이 '('일 때까지 계속 출력을 한다.</p>
