...
### 문제 풀이
 <p>이 문제는 누적합과 이분탐색으로 푸는 문제이다. 먼저 A와 B에서 그 배열에 나올 수 있는 모든 합을 구한다. 이는 이중문과 누적합을 이용하여 구한다. 구한 수들은 sumA, sumB에 저장한다. 그 다음 T에 각 sumA를 빼준다. 빼준 값은 sumB에서 찾는데 찾을 때 중복이 있을 수 있으니 lower_bound와 upper_bound를 이용하여 구하려는 요소의 개수를 구해줘서 답의 개수를 더해준다.</p>
