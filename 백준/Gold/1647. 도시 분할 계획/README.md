...
<p>이 문제는 길은 어느 방향으로 다닐 수 있고 임의의 두 집 사이에 경로가 존재하며 집들이 서로 연결되어 있다고 한다. 이 길을 유지하기 위해서는 유지비의 값이 드는데 이 값을 최솟값으로 구하려고 한다. 이렇게 하면 최소 스패닝 트리문제로 보이지만 여기서 조건이 더 붙여야한다. 그것은 이장이 마을을 두 개의 분리된 마을로 분할한다고 하기 때문에 유지비 중에서 가장 큰 값을 빼주면 답을 구할 수 있다.</p>
