...
### 문제 풀이
 <p>이 문제는 모든 노드에서 모든 노드로 구하는 문제로 플로이드 워샬로 구해야한다. 하지만 여기서 원하는 답은 가장 먼저 거쳐야 하는 집하장이다. 그렇기 때문에 단거리를 구하기 위한 배열과 가장 먼저 거쳐야 하는 집하장 배열을 따로 구해줘야 한다. 그래서 처음에 집하장 배열은 a에서 b로 가는 경로에는 b로 잡는다. 그 후, 플로이드 워샬로 단거리를 구하면서 먼저 거쳐야 집하장을 업데이트한다. </p>
