...
### 문제 풀이
 <p>이 문제는 이분탐색으로 가능한 거리를 크게 설치해야하기 때문에 low는 1로 high는 주어진 값중에 가장 큰 값으로 선정하여 이분탐색을 하여 거리가 알맞은지 그리고 C만큼 설치할 수 있는지 확인하여 문제를 푼다.</p>
