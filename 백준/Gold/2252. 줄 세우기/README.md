...
### 문제풀이
 ...
 <p> 이 문제는 위상정렬 문제로 순서가 정해져있는 작업을 차례로 수행해야 할 때 그 순서를 결정해주기 위해 사용하는 알고리즘이다. 먼저 해야할 것들을 큐에 넣고 큐를 빼주면서 뺀 노드와 연결된 간선들을 지운다. 그리고 간선들이 빼고 난 뒤 연결된 간선이 0인 노드가 있으면 큐에 추가한다. 그런식으로 반복하여 answer의 갯수가 N개이면 출력한다. </p>
