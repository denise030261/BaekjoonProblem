...
### 출력 
 ...
 ### 문제 풀이
  <p>이 문제는 그리디로 우선순위 큐와 함께 사용해야한다. 각 강연은 d일 안에만 수행하면 된다. 그래서 for문으로 1일부터 ~ 마지막 날까지 돌면서 해당 날의 강연을 우선순위 큐에 넣는다. 이 때 우선순위 큐의 크기가 현재 날짜보다 더 크면 하루에 하나인데 현재 가능한 강연수보다 더 크므로 강연료의 최솟값을 빼준다. 그래서 for문이 끝나면 큐에 있는 모든 강연료를 더해줘서 출력을 한다.</p>
