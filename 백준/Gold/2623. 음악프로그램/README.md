...
### 문제 풀이
 <p>이 문제는 비순환 단방향 그래프에서 순서가 정해져 있는 작업을 차례대로 수행하는 위상 정렬을 사용하면 문제를 풀면 된다. 그래서 각 보조 PD가 짠 순서를 이용하여 간선을 연결하고 진입 차수를 더해준다. 그렇게해서 진입차수가 0인 노드부터 큐에 배치를 하며 0인 가수와 간선으로 연결된 가수들을 간선 해제하며 진입차수를 낮춘다. 또한 진입차수가 0이 되면 큐에 배치하여 이렇게 전체 가수의 순서를 정해준다. 순서를 배치를 하는데 N과 같지 않으면 불가능한 경우이기 때문에 0을 출력한다.</p>
