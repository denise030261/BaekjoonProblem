...
### 문제 풀이
 <p>최소한의 움직임으로 아이들을 순서대로 줄을 세워야하기 때문에 오름차순으로 정렬을 해야 한다. 
  그러기 위해서는 먼저 오름차순으로 배정되어 있는 아이들을 파악한다. 파악하기 위해서 LIS(최장 부분 증가 수열) 알고리즘을 사용한다.
  그러면 남은 아이들은 순서에 맞게 움직이면 되기 때문에 전체 아이들 수에 LIS를 뺀 수가 답이다.</p>
