...
### 문제 풀이
 <p>이 문제는 dp와 누적합을 이용하여 푸는 문제이지만 본인은 dp와 슬라이싱 윈도우를 이용하여 문제를 풀었다. 슬라이싱 윈도우로 현재와 M의 개수의 합까지 더해 합을 구해준다. dp는 2차원 배열을 만들어 1차원은 현재 객차, 2차원은 소형 기관차의 개수를 이용하여 dp[i][j] = max(dp[i-1][j], dp[i-M][j-1]+sums[i])를 이용하여 문제를 풀었다. </p>
