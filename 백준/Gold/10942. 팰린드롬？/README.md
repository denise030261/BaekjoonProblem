...
### 문제 풀이
 ...
 <p>이 문제는 dp문제로 처음에는 한글자와 이전 순서의 숫자가 맞으면 팰린드롬이기 때문에 true 처리를 해준다. 그리고 3개 이상일 때는 처음 숫자와 끝 숫자가 맞고 그 사이가 팰린드롬이면 맞다고 처리한다. 그 사이가 팰린드롬인지 판단을 하기 위해서는 기준의 다음 순서이면서 끝 이전 순서의 dp를 확인하면 된다. </p>
