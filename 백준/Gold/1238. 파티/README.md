...
### 문제 풀이
 ...
 <p>이 문제는 각 마을에서부터 X번 마을까지, X번 마을에서 각 마을까지의 최단거리를 구하고 그 중에서 가장 오래 걸리는 학생의 소요시간을 구하는 것이기 때문에 최단거리 알고리즘인 폴로이드-와샬 알고리즘을 사용하여 문제를 풀었다. 각 노드를 거치는 경우들을 사용하여 각 노드에서 노드까지의 최단거리를 구하였다. 이 문제는 폴로이드-와샬 알고리즘을 사용하지 않아도 다익스트라 알고리즘을 사용해도 되고 오히려 더 효율적이다. 폴로이드-와샬 알고리즘은 모든 정점에서 모든 정점으로의 최단 경로를 구해야하는 경우에 사용되며 O(n^3)이다.</p>
