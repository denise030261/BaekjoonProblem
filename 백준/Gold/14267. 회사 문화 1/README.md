...
### 문제 풀이
 <p>이 문제는 dp문제와 dfs를 사용해서 푸는 문제이지만 본인은 dp와 bfs를 사용해서 문제를 풀었다. 입력을 받을 때 상사의 vector에 부하들을 push하는 형태이고 사장님들을 따로 벡터에 넣는다. 그래서 사장님으로부터 제일 아래에 있는 부하까지 dp식으로 칭찬을 더해준다.</p>
