...
### 문제 풀이
 <p>이 문제는 구현 문제로 스택을 이용하여 문제를 풀었다. 스택을 쌓아서 숫자를 만나면 아예 다른 숫자 변수에 넣고 빈 칸을 만나게 된다면 자리수를 늘려서 그 숫자 변수에 넣어준다. 그 다음 연산자가 무엇이냐에 따라서 그 변수에 연산자를 붙여서 최종합에 더해준다. 그렇게해서 최종합이 0이면 그 과정을 set 자료구조에 넣어 자동으로 정렬되게 하고 답을 도출한다. </p>
