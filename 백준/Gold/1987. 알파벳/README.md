...
### 문제 풀이
 ...
 <p>같은 알파벳이 적힌 칸을 두 번 지날 수 없는 대신에 말이 지날 수 있는 최대의 칸 수를 출력해야 한다. 이는 visited 배열을 알파벳 순서로 지정하여 알파벳 지나는 곳에서 visited를 true 처리하여 DFS로 말을 지나가게 한다. 그러면 같은 알파벳을 지날 수 없게 만들 수 있다.</p>
