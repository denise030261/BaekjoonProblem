...
### 문제 풀이
 <p>이 문제는 비트마스킹과 브루트포스를 이용해서 문제를 풀어야한다. 처음에는 그래프 탐색을 이용해서 문제를 풀려고 했지만 가짓수로 나누기 힘들고 탐색 방법이 생각나지 않았다. 그 대신에 비트마스킹을 통해서 가로, 세로 부분을 나눠서 종이조각을 나눈다. 이 때 가로, 세로는 비트마스킹을 통해서 나눠준다. 그래서 가로열 따로, 세로열 따로 구해서 연속적이라면 숫자를 붙여주고 다른 부분을 만났을 때 또는 끝까지 갔을 때 그 때 수로 변환해서 값을 더해준다. 이렇게 하지 않으면 어떤 부분을 가로로 할지 세로로할지 구분할 수 없어 비트마스킹으로 풀어야한다. </p>
