...
### 문제 풀이
<p>이 문제를 풀기 위해서 BFS 알고리즘을 사용하였다. 치즈를 녹기 위해서는 공기와 두 변에 접촉을 해야한다. 대신에 이 공기는 외부 공기이어야하므로 외부공기와 내부공기를 나누어야한다. 그렇게 BFS로 공기를 나누고 외부 공기가 2변 이상 접촉할 때 녹는 로직을 BFS로 세워서 치즈가 모두 녹아 없어지는 시간을 구하였다.</p>
