...
### 문제 풀이
 <p>이 문제는 구현 문제로 이동하는 구현, 주사위 굴리는 구현, 점수 얻는 구현으로 크게 3가지 방법으로 구현하였다. 이동할 때를 위해서 방향의 변수를 잡아서 이동하게 하고 만약에 칸 밖이면 반대 방향으로 가게 구현했다. 주사위의 경우에는 따로 배열을 구해서 오른쪽으로 굴릴 때, 왼쪽으로 굴릴 때, 위로 굴릴 때, 아래로 굴릴 때 어떻게 배열이 달라지는지 확인하고 구현하였다. 그 다음 점수는 이동하는 위치에 있는 cost가 같은 칸들을 bfs 통해서 몇 개있는지 확인하고 점수를 구현하였다. 마지막으로 주사위 바닥과 이동 칸의 점수와 비교하여 방향을 결정하도록 구현하였다. </p>
