...
### 문제 풀이
 ...
 <p>이 문제는 S에서 T를 만드는 것보다는 T에서 S를 만들면서 문자를 빼야하는 문제이다. T의 끝문자부터 A가 나오면 그대로 빼주고 B가 나오면 B를 뺀 다음에 문자열을 뒤집어줘야한다. 그 이유는 B의 문자를 붙일 때 문자열을 뒤집고 추가하니까 그 반대로 실행하면 된다.</p>
