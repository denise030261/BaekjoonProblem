...
### 문제 풀이
 <p>이 문제는 속하지 않은 학생들의 수를 계산하는 문제로 dfs로 풀었다. 처음에는 출발점을 순환의 출발점이라고 가정을 하고 풀었다. 하지만 그렇게 해버리면 시간 초과가 나기 때문에 connect 배열을 이용하여 0은 아예 연결이 되지 않은 학생, 1은 현재 dfs중인 학생, 2는 이미 순환이 완료된 학생으로 구분을 하여 풀었다. 그래서 connect가 0인 학생을 기점으로 재귀를 하다가 connect가 1인 학생을 만나면 그 때부터 순환이 시작된다는 의미다. 그래서 그 이후로 같은 팀의 학생의 수를 계산해준다. 이렇게 해야 하는 이유는 출발점을 순환의 출발점이라고 가정해버리면 중간의 순환이 있어도 이를 무시하고 나중에 또 재귀를 해야하기 때문이다. </p>
