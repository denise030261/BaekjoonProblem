...
### 문제 풀이
 <p>'ㅗ' 블록을 제외한 나머지 블록은 방향이 갈리는 블록이 아니기 때문에 DFS를 통해서 테트로미노가 쌓인 칸에 수를 더하여 값을 구한다. 'ㅗ'자 모양의 테트로미노는 '+'모양으로 합을 구한 뒤 그 중에서 가장 작은 값을 빼서 테트로미노를 구한다. 이렇게 구한 테트로미노 값 중 합의 최댓값을 출력한다. </p>
