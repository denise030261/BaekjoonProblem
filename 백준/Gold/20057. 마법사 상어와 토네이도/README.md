...
### 문제 풀이
 <p>이 문제는 그대로 구현하면 된다. 비율대로 모래를 뿌리는 것은 dx, dy 변수를 통해서 문제를 풀면 된다. 또한 계속해서 이동하는 것이기 때문에 다음 지점이랑 방향이 바뀌는 지점 그리고 얼마큼 가서 방향을 바꿀지에 대해서 고민을 하고 문제를 풀면 된다.</p>
