...
### 문제 풀이
 <p>이 문제는 위상정렬뿐만 아니라 dp를 사용해야한다. 위상정렬을 사용해야하는 이유는 일단 순환 그래프가 아니고 건물을 짓기 위해서는 순서가 필요하기 때문에 위상정렬을 사용해야한다. 그리고 각 건물들을 짓기 위해서 최소 시간을 출력을 해야한다. 최소 시간을 구해야하지만 답을 구할 때는 dp[다음] = max(dp[다음], dp[현재]+시간[과거])로 구해야한다. 그 이유는 특정 건물을 짓기 위해서 다른 건물을 먼저 지어야하는데 그 건물들끼리 연결될 수도 있고 연결이 되지 않을 수 있다. 그래서 위상정렬로 인해 순서는 확정적이고 최대값은 이미 연결된 건물은 지었다는 의미이므로 이와 같은 식으로 구해야한다. </p>
