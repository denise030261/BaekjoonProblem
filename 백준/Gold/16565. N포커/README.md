...
### 문제 출력
 <p>이 문제는 조합을 생각해야한다. 포카드를 만들 숫자의 조합과 그 나머지를 고르는 조합을 곱해야하는데 문제는 이들이 겹칠 수 있다. 중복이 되는 경우가 있기 때문에 k가 짝수일 때만 빼준다. 이 때 조심해야할 것이 모듈러 연산인데 모듈러 연산은 나누기에서는 적용이 안되고 뺄셈도 조심해야하기 때문에 조합 계산할 때 파스칼의 삼각형을 사용하는 것이 올바른 계산할 수 있다. </p>
