...
### 문제 풀이
 <p>이 문제 풀이는 dp로 3차원 배열을 이용해서 min으로 크기를 비교하여 문제를 풀었지만 문제는 한 번의 공격에서 같은 SCV를 여러 번 공격할 수 없는 점 때문에 dp로만으로는 풀지 못한다. 그래서 bfs를 이용하여 세 개의 공격하는 패턴을 구해서 현재 상태에서 패턴별로 공격을 하고 visited을 통해서 체력 상태의 방문 여부를 판단하여 cost를 구해준다. </p>
