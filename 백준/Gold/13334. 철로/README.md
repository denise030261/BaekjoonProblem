...
### 문제 풀이
 <p>이 문제는 정렬을 하면서 스위핑 알고리즘을 사용하여 푸는 문제이다. 정렬을 끝점을 기준으로 오름차순으로 정렬한다. 이 때 출발점과 끝점은 무조건 오름차순이 아니기 때문에 이를 조심해야 한다. 그리고 스위핑 알고리즘을 사용하여 선분에 포함된느 사람들의 최대 수를 출력하면 되는데 이는 우선순위 큐를 사용한다. 우선순위 큐로 힙의 첫번째 원소가 현재 위치의 도착점에 d를 뺀 값보다 작으면 오른쪽으로 밀어야하기 때문에 pop을 해주고 힙의 첫 번째 원소가 크거나 같을 때 그 때 push를 해줘야한다. 그래서 현재 사람들의 최대수를 게속 업데이트를 하면서 답을 구해준다. </p>
