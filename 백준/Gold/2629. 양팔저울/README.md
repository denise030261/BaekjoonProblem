...
### 문제 풀이
 <p>이 문제는 dfs와 dp를 사용해서 푸는 문제이다. 처음에는 dfs으로만 또는 dp로만 풀려고 했지만 풀리지 않았다. 그래서 이를 둘 다 사용하여 오른쪽에 추만 두는 경우, 건너뛰는 경우, 왼쪽에 두는 경우(이는 빼는 것으로 구현했다)로 dfs를 구현하여 문제를 풀었다. </p>
