...
### 문제 풀이
 <p>이 문제는 위상정렬 문제로, 순서가 정해져있는 작업을 차례로 수행해야 할 때 그 순서를 정해주기 위해 사용하는 알고리즘이다. 처음에는 진입차수를 만들어야하는데 진입차수는 들어오는 간선의 수를 저장하는 것이다. 만약에 0이면 출발 건물이므로 큐에 저장한다. 그래서 큐에 있는 건물을 건설하면서 그 건물와 연결되어있는 건물들의 진입차수를 빼준다. 이 때 0이 되면 큐에 넣는다. 시간의 경우에는 동시에 짓는 것이 가능하기 때문에 최대 건설 시간으로 지정한다. 그래서 건설해야 할 건물의 번호까지 도달하면 지금까지 더한 최대 건설 시간을 출력한다.</p>
