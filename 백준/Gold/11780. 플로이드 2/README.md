...
### 문제 풀이 
<p>이 문제는 모든 정점에서 출발하여 모든 정점을 도착하는 최소의 비용을 구하는 문제로 플로이드와샬을 사용해야한다. 그리고 그 경로로 갔을 때 중간에 지나가는 정점도 출력을 해야하는데 vector의 2차원 배열을 사용하였다. 그리고 비용의 값을 업데이트할 때마다 먼저 벡터(출발->도착)에 값이 존재하는지 확인하고 만약에 있으면 이를 지우고 다시 업데이트한다. 그래서 출발->중간의 벡터의 원소값들, 중간값, 중간->끝의 벡터의 원소값들로 업데이트한다. </p>
