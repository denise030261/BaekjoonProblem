...
<p>이 문제 풀이는 dp 문제로 가방 알고리즘을 사용하여 풀면 된다. 그래서 dp[학생][블록 높이]로 둔다. 학생의 순서는 순서대로이니 각 순서대로 1~H까지 학생이 가지고 있는 블록을 가지고서 블록 높이를 H-(학생이 가지고 있는 높이)로 구해준다.</p>
