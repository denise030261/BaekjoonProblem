...
### 문제 풀이
 ...
 <p>이 문제는 단순 구현 문제로 과정을 요약하면 1. 처음에는 벨트와 로봇이 같이 움직인다. 2. 가장 먼저 올려진 로봇부터, 벨트가 회전하는 방향으로 한 칸 이동할 수 있으면 움직인다(이동하기 위해서는 이동하려는 칸에 로봇이 없고 내구도가 1이상이어야 한다. 또한 벨트의 N번째 칸에 있을 때 내려진다. 즉, 로봇이 없어진다.) 3. 벨트의 1번째 칸이 내구도가 0이 아니면 로봇을 올린다. 4. 내구도가 0인 칸의 개수가 K개 이상일 때 과정을 종료한다. 그래서 종료할 때 몇 번의 단계를 거쳤는지 출력하면 되는 문제이다. </p>
