...
### 문제 풀이
 <p>이 문제는 LIS로 풀어야한다. 그 이유는 꼬이지 않을려면 오름차순으로 배치를 해야하기 때문에 LIS 알고리즘을 사용해야 한다. 입력수가 10만이기 때문에 dp가 아닌 이분탐색을 사용해서 LIS로 구해야 한다. 이분탐색으로 풀기 위해서 배열을 하나 더 만들고 반복문을 통해서 배치를 해야한다. 배열의 끝이 현재 순서보다 작으면 넣고 현재 순서가 작으면 이분탐색을 통해서 조건에 맞도록 최대한 작은 부분에 교체를 한다.  </p>
