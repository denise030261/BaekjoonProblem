...
### 문제 풀이
 <p>이 문제는 tsp 알고리즘을 사용하여 문제를 풀어야한다. 비트마스킹과 dp를 사용하는 것까지는 알아냈지만 dp의 사용법을 3차원으로 사용해야하는줄 알았지만 아니다. 어처피 1번으로 출발하든 2번으로 출발하든 순회를 하기 때문에 다시 되돌아온다. 그래서 이는 신경 쓰지 않아도 되기 때문에 2차원으로 사용하여 1차원은 현재는 어디에 있는지 2차원은 어떤 장소를 방문했는지 나타나여 가장 적은 비용을 들이는 순회 여행을 구한다. 그리고 top-down 방식으로 구하였다. </p>
