...
### 문제 풀이
 <p>이 문제는 최소 스패닝 문제로 처음에는 각 별과의 거리들을 구해준다. 이들을 간선으로 취급하여 모든 정점으로부터의 총합 단거리를 구해준다. 그 때 사용하는 알고리즘으로 프림 알고리즘으로 다익스트라 알고리즘과 헷갈리면 안된다. 다익스트라로 구했을 때는 우선순위 큐를 구했을 때 처음 정점으로부터 지금까지의 경로의 값을 합한 값을 저장하면서 구했지만 프림은 각 정점으로부터 방문만 하면 되기 때문에 현재 정점의 값만을 저장해줘서 누적으로 합해주면 된다. </p>
