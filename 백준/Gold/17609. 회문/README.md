...
### 문제 풀이
 <p>이 문제는 팰린드롬 문제로 투 포인터를 이용해야한다. 그래야 맨 처음과 맨 끝을 비교해서 가운데에 만나도록 더하거나 빼는데 쭉 했는데 맞으면 회문이고 만약에 한 번 틀리면 앞에서 한 번 더 땡길건지 뒤에서 땡길건지 결정한다. 결정하는 방법은 땡긴 상태에서 쭉 검사를 했는데 맞으면 그 방법으로 진행한다. 만약 둘 다 성립되지 않는다면 2를 출력해야 한다.</p>
