...
### 출력 
 ...
 ### 문제 풀이
  <p>이 문제는 그래프 탐색으로 dfs나 bfs로 풀려고 했지만 그렇게 하면 한 경로로 쭉 가야되기 때문에 이 방법으로 풀면 안된다. 그래서 직접적으로 자리를 선점해야하는데 이 때 pair<int,int> 방식으로 하면 메모리 초과가 나기 때문에 <int>로 index 버전으로 선점을 했다. 그래서 x=index/5, y=index%5로 해서 S가 몇 개인지 확인하고 과반수면 그 자리들이 연결되었는지 확인하고 그것까지 확인되면 경우의 수를 포함한다. </int> </p>
