...
### 문제 풀이
 <p>이 문제는 처음에 dp로 풀었으나 문제는 칸에 도착할 때 사다리나 뱀이 있으면 무조건 이동해야하는데 이 때 뱀이 문제가 되는 것이다. 뱀은 이전으로 돌아가는 것이기 때문에 dp로 풀면 안되는 것이다. 대신 bfs를 통해서 1~6칸 다 이동하되 중간에 사다리나 뱀이 있으면 무조건 이동하는 방식으로 구현을 하였다.</p>
