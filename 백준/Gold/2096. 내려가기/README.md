...
### 문제 풀이
 ...
 <p> 그림에 나와있는대로 세 개의 숫자 중에서 하나를 고를 때 어느 위치에 있느냐에 따라 내려가는 것 제한된다. 그래서 입력 받을 때마다 각 위치에 최솟값과 최댓값을 구하여 최종의 최솟값과 최댓값을 구한다. 각 위치에서 본인 위치의 값과 위치에 따라 더할 수 있는 값 중에서 최댓값 또는 최솟값을 더해 최종적으로 답을 도출하는 이 문제는 dp문제인 것이다. </p>
