...
### 문제 풀이
 <p>그리디, 정렬, 우선순위 문제임을 눈치챘지만 이를 어떻게 사용할까 고민이였다. 데드라인을 기준으로 할지 컵라면을 기준으로 할지 몰랐지만 데드라인을 먼저 하고 우선순위 큐를 넣는다. 큐의 크기는 결국 문제 푼 시간이기 때문에 먼저 큐에 넣고 큐의 사이즈와 데드라인 시간을 비교하고 큐 사이즈가 더 크면 큐에서 가장 적은 컵라면 수를 뺀다. 그래서 이를 우선순위 큐를 사용하여 문제를 푼다. </p>
