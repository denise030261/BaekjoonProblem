...
### 문제 풀이
 <p>이 문제는 문자열로 현재 상태가 0이 몇 개고 1이 몇 개인지에 따라서 엔진 소리인지 아닌지 판별하되 0 부분에서 100~1~에서 1이 몇 개냐에 따라 01이 될 수도 있고 100이 될 수 있기 때문에 이를 주의하며 문제를 풀어야한다. 정규표현식 라이브러리가 있지만 이를 사용하지 않고 문제를 풀었다.</p>
