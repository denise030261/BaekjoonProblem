...
### 문제 풀이
  ...
  <p>이 문제는 dp로 최소의 힘을 출력해야 한다. BFS로 풀었는데 그러면 메모리 부족이 떠서 다른 방법으로 푼 것이 dp 방법이다. dp 방법으로 각각 현재 단계에서 왼쪽 발을 어디에 두나, 오른쪽 발을 어디에 두나에 따라서 그 값의 최솟값을 비교한다. 그 단계가 최종 단계까지 거쳐서 최소의 힘으로 둘 수 있는 값을 출력하는 것이다</p>
