...
### 문제 풀이
 <p>이 문제는 구현으로 다리 놓고 직접 게임을 플레이하는 것을 vector를 사용하려고 했으나 그러면 해제를 했을 때 문제가 되어 직접 배열로 구현을 하였다. 또한, 연속적으로 배치가 되면 안되기 때문에 앞뒤 다리가 놓여있는지 확인을 해야한다. 범위를 순회할 때 겹치는 부분이 없는지 확인을 해야한다. </p>
