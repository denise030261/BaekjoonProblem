...
<p> 이 문제는 그래프 문제로 본인은 DFS 방법으로 풀었다. 구해야하는 케이스가 적록색약 아닌 사람이 볼 때와 적록색약 사람이 볼 때의 구역이기 때문에 DFS도 2가지의 방법을 구해서 하나는 R,G,B 모두 구분하는 방법, 다른 하나는 (R,G), B로 R과 G를 같은 취급을 하여 구분하는 방법으로 문제를 풀었다.</p>
