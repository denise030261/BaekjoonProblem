...
### 문제 풀이
 <p>이 문제는 정렬을 하고 누적합은 아니지만 그와 유사한 방식으로 풀어야 한다. 조합을 구하는 것이고 최솟값과 최대값의 차이를 구하는 것이기 때문에 먼저 정렬을 취해준다. 정렬을 한 뒤에는 각 조합별로 최대최소 차이를 구해야하는데 다른 관점으로 문제를 풀었다. 이는 어떤 특정값이 최대가 될려면 그 작은 값들로 부분집합으로 구성되어있어한다. 즉, 특정 숫자가 작은 값들이 3개면 그 부분집합은 2^3이다. 반대로 어떤 특정값이 최소가 될려면 큰 값들로 부분집합으로 구성되어야하기 때문에 2^(n-1-i)으로 구해야한다. 그러면 특정값보다 작은 값들의 부분집합은 최대값이기 때문에 그 개수를 더해주고 큰 값들의 부분집합은 최솟값이기 때문에 그 개수를 빼줘서 모든 특정값의 부분집합의 차이의 합을 구해준다. </p>
