...
### 문제 풀이
 <p>이 문제는 비트마스킹으로 풀면 좋지만 dfs를 사용해서 풀었다. 근데 dfs로 풀면 조심해야할 사항이 많기 때문에 구현 난이도가 높지만 비트마스킹으로 풀면 안정성이 높고 입력값이 높지 않기 때문에 비트 마스킹으로 풀어야한다. 비트마스킹으로 각 쌍들을 비트로 나타내서 그 부분이 1이면 지운다. 지우기 위해 remove 배열에서 true처리하여 문자열을 출력할 때 각 인덱스가 지우는 부분인지 확인하고 출력한다.</p>
