...
### 문제 풀이
 ...
 <p>이 문제는 단순 구현문제로 길을 확인할 때 먼저 높이차가 1인지 판단을 한다. 만약 올라가게 된다면 count가 L보다 크거나 같으면 count=1로 초기화고 아니면 낮은 칸이 L개 연속되지 않은 의미이기 때문에 다음 줄로 넘긴다. 내려간다면 count가 0보다 크거나 같으면 count를 -(L-1)로 바꾼다. 그 이유는 내려가면 낮은 칸이 되고 L만큼 연속되는지 판단을 하기 위함이다. 만약 같은 칸을 가면 coount를 더한다. 마지막 칸에서는 내려가는 경우가 있기 때문에 L이 1보다 크면 경사로 못 만든다고 취급하고 이전 칸과 같다면 count가 L보다 크거나 같은지 판단을 하여 구현하였다.</p>
