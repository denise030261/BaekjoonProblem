...
### 문제 풀이
 ...
 <p>이 문제를 보면 선분을 그려서 사이클이 완성되는 순간 게임이 종료된다. 즉, 무방향 그래프임을 확인할 수 있다. 무방향 그래프에서 사이클을 판별하려면 Union-Find나 DFS 알고리즘 문제를 풀어야 하지만 본인은 Union-Find로 풀었다. 모든 노드들의 부모를 자기 자신으로 초기화한 뒤에 노드의 입력을 받는다. 입력을 받고 각 노드의 부모 노드가 같은지 다른지에 따라서 같으면 사이클이 발생한 것이고 다르면 두 노드에 union 연산을 수행한다. union 연산으로 노드의 크기를 비교해서 작으면 부모 노드로 선정하였다. </p>
