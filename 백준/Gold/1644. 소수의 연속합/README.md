...
### 문제 풀이
 ...
 <p>이 문제는 투 포인터 문제로 먼저 에라토스테네스의 체를 이용해서 소수를 골라준다. 그 이후 순서대로의 소수의 합을 저장하고 투 포인터를 이용해서 소수의 합이 N인 경우의 수를 구한다. 투 포인터를 이용할 때 start, end 동시에 움직여야 한다. 왜냐하면 한 쪽만 움직이다가 빠지는 경우가 생길 수 있기 때문이다.</p>
