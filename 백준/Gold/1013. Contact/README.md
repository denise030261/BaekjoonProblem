...
### 문제 풀이
 <p>이 문제는 정규 표현식으로 풀어도 되는 문제지만 문자열을 이용하여 문제를 풀었다. 뒤에서부터 시작하여 0이 올 때 누적된 0과 1이 없으면 바로 NO처리를한다. 1이 올 때 이전에 0,1이 하나씩 있으면 리셋을 하고 1이 1개 이상, 0이 2개 이상이면 리셋해주고 다음으로 넘어가고 0은 하나지만 1은 2개 이상이면 NO처리를 해준다. 그렇게 해서 1과0이 하나만 남아있거나 둘 다 없으면 true처리를 하지만 아니면 false 처리를 한다. 간단한 방법으로는 <regex>를 사용하여 정규 표현식으로 문제를 해결해도 된다. </p>
